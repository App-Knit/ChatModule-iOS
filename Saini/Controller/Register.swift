...
class Register: UIViewController,UIImagePickerControllerDelegate,UINavigationControllerDelegate,UITextFieldDelegate,CLLocationManagerDelegate {
    ...
    @IBAction func registerBtnPressed(_ sender: UIButton) {
        self.view.endEditing(true)
        if userImage.image == #imageLiteral(resourceName: "upload_img"){
            ProjectManager.sharedInstance.showAlertwithTitle(title: "Error", desc: "Please select your profile picture.", vc: self)
        }
        else if firstNameTxtField.text!.count <= 0 || (firstNameTxtField.text?.trimmingCharacters(in: .whitespaces).isEmpty)! {
            ...
            ProjectManager.sharedInstance.showAlertwithTitle(title: "Error", desc: "Saini unable to find your current location.", vc: self)
        }
        else{
            register()
        }
    ...
    func register(){
        User.registerUser(withName: firstNameTxtField.text! + " " + lastNametxtField.text!, email: emailTxtField.text!, password: passwordTxtField.text!, phoneNumber: "+" + countryCodeTxtField.text! +  phoneTxtField.text!, profilePic: userImage.image!, location: position, loginHandler: {(Loginhandler) in
            if Loginhandler == nil{
                let storyboard = UIStoryboard(name: "Main", bundle: nil)
                let Login = storyboard.instantiateViewController(withIdentifier: "Login") as! Login
                let alert = UIAlertController(title: "Verification", message: "A verification mail has been sent to your registered mail id please verify the email.", preferredStyle: UIAlertControllerStyle.alert)
